...
def copy_stack(old_xl, suffix, recursive=False):
	new_name = f"{old_xl.name}-{suffix}" if recursive else suffix
	...
	if new_xl.type == "storage/posix":
		new_xl.opts["directory"] += f"/{suffix}"
	return new_xl
...
def cleanup(parent, graph):
	if parent.type in good_xlators:
		...
		if parent.type == "protocol/server":
			...
			parent.opts["transport-type"] = "ssl"
		sv = [cleanup(child, graph) for child in parent.subvols]
		parent.subvols = sv
	else:
...
def load(path):
	...
	fp = file(path, "r") if type(path) in (str,) else path
	all_xlators = {}
	xlator = None
	last_xlator = None
	while True:
		...
		if text[0] == "end-volume":
			...
			xlator = None
			continue
		raise RuntimeError(f"unrecognized keyword {text[0]}")
	if xlator:
...
def generate(graph, last, stream=sys.stdout):
	for sv in last.subvols:
		if not sv.dumped:
			...
			print("", file=stream)
			sv.dumped = True
	print(f"volume {last.name}", file=stream)
	print(f"    type {last.type}", file=stream)
	for k, v in last.opts.items():
		print(f"    option {k} {v}", file=stream)
	if last.subvols:
...
def push_filter(graph, old_xl, filt_type, opts={}):
	...
	if len(old_xl.name) > len(suffix) and old_xl.name[-len(suffix) :] == suffix:
